...
export class AuthController {
 ...
 @Post('/auth/retailerLogin')
  ...
  async sendEmployeeTxtMsg(@Body() sendEmailDto: SendOTPEmailMessageDto): Promise<IResponse> {
    ...
    const inputString = otp.toString();
    const lastasterisk = await this.helperService.getLastSixDigits(inputString);
    await this.authservice.updateUserOtpByMobile({mobile : mobile, otp : otp.toString()});
...
async newupdateOtponBoth(@Body() newcustomerlogininfoDTO: newcustomerlogininfoDTO): Promise<IResponse> {
    try {
        const { otp, mobile, email } = newcustomerlogininfoDTO;
        
        // Call the authservice method to fetch user data
        const user = await this.authservice.newupdateOtponBoth(newcustomerlogininfoDTO);
